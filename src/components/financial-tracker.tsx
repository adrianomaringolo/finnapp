...
import { CircleDollarSign, Heart, ShoppingCart, Check, Edit, Trash2 } from "lucide-react"
...
const TransactionItem = ({ transaction, onEdit, onRemove }: { transaction: Transaction; onEdit: () => void; onRemove: () => void }) => {
  const { type, description, amount } = transaction
  const icons = {
    income: <CircleDollarSign className="w-6 h-6 text-green-500" />,
    ...
    <Dialog>
        ...
        <div className="flex items-center justify-between py-3 cursor-pointer hover:bg-muted/50 rounded-md px-2">
          ...
          <div className="flex items-center gap-2">
            <span className={`text-lg font-medium ${amountColor}`}>
              {type === "income" ? "" : "-"}R$ {amount ? amount.toFixed(2).replace(".", ",") : "0,00"}
            </span>
      ...
      <DialogContent>
        ...
        <div className="py-4">
          ...
          <p><strong>Amount:</strong> R$ {amount ? amount.toFixed(2).replace(".", ",") : "0,00"}</p>
        </div>
        <DialogFooter>
...
const DayGroup = ({ date, transactions, onEdit, onRemove }: DayGroup & { onEdit: (id: string) => void; onRemove: (id: string) => void }) => {
  const balance = transactions.reduce((acc, curr) => 
    acc + (curr.type === "income" ? (curr.amount || 0) : -(curr.amount || 0)), 0
  )
    ...
    <div className="mb-6">
      ...
      <div className="flex justify-end mt-2">
        <span className="text-lg">
          Saldo do dia: {balance >= 0 ? "" : "-"}R$ {Math.abs(balance).toFixed(2).replace(".", ",")}
        </span>
      </div>
    </div>
  )
  ...
  return (
    <div className="space-y-2">
      <div className="flex justify-between items-center">
        ...
        <span className="text-green-500">R$ {income.toFixed(2).replace(".", ",")}</span>
      </div>
      <div className="flex justify-between items-center">
        <div className="flex items-center gap-2">
...
export function FinancialTracker() {
  ...
  const groupedTransactions: Record<string, Transaction[]> = transactions.reduce((acc, transaction) => {
